...
<div id="top_banner" style="background-image :url('{{ asset('image/img1.jpg') }}')">
    <div class="contener">
        <h1 class="main_title">
            <span class="part1">Don’t miss out!</span>
            <span class="part2">
                Explore the  
                <span class="colored_part">vibrant events</span>
                happening locally and globally.
                </span>
        </h1>
        <div class="search">
            <form method="get"  action="{{ route('products.search') }}" id="search">
                <span class="material-icons search_icon">search</span>
                <input class="search_input" type="text" name="search" placeholder="Search Event,Category,location ..."/><!--
                --><div class="selec_box">
                        <div class="icon_loc_div"><span class="material-icons form_icon">location_on</span></div>
...
<article id="categories" class="section_gap">
    <div class="contener">
        <h2 class="title">Explore Categories</h2>
        <div class="categories_box">
            @foreach ($categories as $category)
            <a class="category_item" href="{{ route('products.category',['categoryId'=>$category->id]) }}" title="{{ $category->name}}">
                <picture class="category_img">
                    <img src="{{ asset('image/'.$category->photo) }}" alt="{{ $category->name }}">
                </picture>
...
<article id="Peoducts" class="section_gap">
    <div class="contener">
       ...
       <div class="see_more_box"><a href="{{ route('events')  }}" title="See More Events">See More</a></div>
    </div>
</artical>
@endsection --}}